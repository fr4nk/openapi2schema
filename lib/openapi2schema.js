...
function getConvertedSchema(type, struct, options) {
	...
	const contentPattern = /application\/(json|.*\+json)/;
	
	if (!(struct.content && Object.keys(struct.content).some(function(key) {return contentPattern.test(key)}))) {
		return {};
	}
	const contentType = Object.keys(struct.content).filter(function(key){
		return contentPattern.test(key);
	})[0];
	
	schema = struct.content[contentType].schema;
